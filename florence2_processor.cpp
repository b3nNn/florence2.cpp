...
namespace Florence2Processor {
    ...
    std::vector<float> Florence2Processor::process(const std::string& text, const cv::Mat& image) {
        ...
        // input_ids, attention_mask, decoder_input_ids: [1, 256]
        ...
        // Run inference
        std::vector<Ort::Value> output_tensors = session.Run(Ort::RunOptions{}, input_names, inputs.data(), inputs.size(), output_names, 1);
